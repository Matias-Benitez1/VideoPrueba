...
public class utils {
	public static void infoTestCase(String funcionality, String description) {
		...
		BaseClass.logger.log(Status.INFO, m);
	...
	public static void eventFailed(String currentEvent, String errorMessage) {
		BaseClass.logger.warning("No se pudo terminar el evento '" + currentEvent + "' a causa de: " + errorMessage);
		...
		BaseClass.logger.log(Status.FAIL, m);
		String path = takeScreenshot(currentEvent);
		try {
			BaseClass.logger.addScreenCaptureFromPath(path, currentEvent);
		} catch (IOException e) {
			e.printStackTrace();
		}
		Assert.fail();
